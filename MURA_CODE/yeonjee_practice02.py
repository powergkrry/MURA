...
view_data_in = Variable(view_data_in.view(-1,350*512)).cuda()
...
view_data_out = Variable(view_data_out.view(-1,350*512)).cuda()
...
train_loader_in = torch.Tensor(train_loader_in.reshape(-1,1,512,350))#.cuda()
print("train_loader_input shape :", type(train_loader_in))
train_loader_input = Variable(train_loader_in.view(-1,512*350)).cuda()
print("loading complete")
...
train_loader_out = torch.Tensor(train_loader_out.reshape(-1,1,512,350))#.cuda()
print("train_loader_output shape :", type(train_loader_out))
train_loader_output = Variable(train_loader_out.view(-1,512*350)).cuda()
...
#    print("encoded :",encoded)
    ...
    for i in range(N_TEST_IMG):
        a[2][i].imshow(np.reshape(decoded_data.data.cpu().numpy()[i],(512,350)), cmap='gray')
        a[2][i].set_xticks(())
        a[2][i].set_yticks(())
    plt.show()
