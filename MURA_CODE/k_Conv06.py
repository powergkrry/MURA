...
class AutoEncoder(nn.Module):
    def __init__(self):
        ...
        self.conv12 = nn.Conv2d(64, 1, 3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        x = self.conv6(x) # 256-256
        x = self.conv6(x) # 256-256
#        x = self.ReLU(self.bn6(self.conv7(x))) # 256-512
#        x = self.conv8(x) # 512-512
#        x = self.conv8(x) # 512-512
#        x = self.conv8(x) # 512-512
#        x = self.ReLU(self.bn5(self.conv9(x))) # 512-256
#        x = self.conv6(x) # 256-256
#        x = self.conv6(x) # 256-256
#        x = self.conv6(x) # 256-256
        x = self.ReLU(self.bn4(self.conv10(x))) # 256-128
#        x = self.conv4(x) # 128-128
        x = self.conv4(x) # 128-128
        x = self.conv4(x) # 128-128
        x = self.ReLU(self.bn3(self.conv11(x))) # 128-64
        x = self.conv2(x) # 64-64
...
autoencoder.load_state_dict(torch.load('./pretrained/k_Conv06_epoch10_model.pth.tar'))
optimizer.load_state_dict(torch.load('./pretrained/k_Conv06_epoch10_optimizer.pth.tar'))
"""
...
test_loader_input = torch.utils.data.DataLoader(
    folder_test_input, batch_size = TEST_BATCH_SIZE, shuffle = False)
...
test_loader_output = torch.utils.data.DataLoader(
    folder_test_output, batch_size = TEST_BATCH_SIZE, shuffle = False)
print("loading complete")
...
# save data
    save_name_model = './pretrained/k_Conv06_epoch' + str(epoch_) + '_model' + '.pth.tar'
    save_name_optimizer = './pretrained/k_Conv06_epoch' + str(epoch_) + '_optimizer' + '.pth.tar'
...
#    scheduler.step()
    ...
    test_loss_output, test_avg_psnr_output = cal_test_mse_psnr(test_file_num, TEST_BATCH_SIZE)
